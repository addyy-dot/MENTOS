    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      {loading ? (
        ...
        <div className="bg-white border border-slate-100 p-12 rounded-3xl text-center shadow-sm max-w-lg mx-auto">
          ...
          <h3 className="text-base font-extrabold text-slate-700">No requests sent yet</h3>
          <p className="text-xs text-slate-400 mt-2">
            Once you request a prep session with a mentor, it will appear here. Find mentors to begin.
          </p>
          <Link
            to="/search"
            className="mt-6 inline-flex px-5 py-3 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold rounded-2xl shadow-md transition-all"
          >
            Search Mentors
          </Link>
        </div>
        ...
        <div className="space-y-6">
          {requests.map((req) => (
            ...
            >
              ...
              {/* Action Side */}
              <div className="flex flex-col justify-center shrink-0 min-w-[160px] md:border-l md:border-slate-50 md:pl-6 gap-3">
                {req.status === 'Scheduled' && req.googleMeetLink && (
                  <a
                    ...
                    className="w-full py-3 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-2xl text-center shadow-lg shadow-emerald-100 hover:shadow-emerald-250 transition-all text-xs flex items-center justify-center gap-2"
                  >
                ...
                {req.status === 'Completed' && (
                  <Link
                    to={`/feedback/${req._id}`}
                    className="w-full py-3 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-2xl text-center shadow-lg shadow-indigo-150 hover:shadow-indigo-255 transition-all text-xs flex items-center justify-center gap-2"
                  >
                ...
                {req.status === 'Accepted' && (
                  <div className="text-center py-2 text-xs font-semibold text-sky-600 flex items-center justify-center gap-1.5">
                    <Clock className="w-4 h-4" /> Awaiting schedule slots
                  </div>
                )}

                {req.status === 'Rejected' && (
                  <div className="text-center py-2 text-xs font-semibold text-rose-500 flex items-center justify-center gap-1.5">
