...
const MenteeDashboard = () => {
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      case 'Pending':
        return <span className="px-2.5 py-1 bg-amber-50 border border-amber-100 text-amber-700 text-xs font-bold rounded-lg uppercase tracking-wide">Pending</span>;
      case 'Accepted':
        return <span className="px-2.5 py-1 bg-sky-50 border border-sky-100 text-sky-700 text-xs font-bold rounded-lg uppercase tracking-wide">Accepted</span>;
      case 'Scheduled':
        return <span className="px-2.5 py-1 bg-emerald-50 border border-emerald-100 text-emerald-700 text-xs font-bold rounded-lg uppercase tracking-wide">Scheduled</span>;
      case 'Completed':
        return <span className="px-2.5 py-1 bg-indigo-50 border border-indigo-100 text-indigo-700 text-xs font-bold rounded-lg uppercase tracking-wide">Completed</span>;
      case 'Rejected':
        return <span className="px-2.5 py-1 bg-rose-50 border border-rose-100 text-rose-700 text-xs font-bold rounded-lg uppercase tracking-wide">Rejected</span>;
      default:
        return <span className="px-2.5 py-1 bg-slate-100 text-slate-700 text-xs font-bold rounded-lg uppercase tracking-wide">{status}</span>;
    }
  };
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      <div className="mb-10 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        <Link
          to="/search"
          className="inline-flex items-center justify-center gap-2 px-5 py-3.5 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-2xl shadow-lg shadow-indigo-100 hover:shadow-indigo-200 transition-all duration-200"
        >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-5 mb-8">
        {[
          { label: 'Total Requests', count: requests.length, icon: <FileText className="w-5 h-5 text-indigo-500" />, bg: 'bg-indigo-50/50' },
          { label: 'Pending', count: statPending, icon: <Clock className="w-5 h-5 text-amber-500" />, bg: 'bg-amber-50/50' },
          { label: 'Scheduled', count: statScheduled, icon: <Calendar className="w-5 h-5 text-emerald-500" />, bg: 'bg-emerald-50/50' },
          { label: 'Completed', count: statCompleted, icon: <CheckCircle className="w-5 h-5 text-violet-500" />, bg: 'bg-violet-50/50' },
        ].map((item, idx) => (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="bg-white border border-slate-100 p-6 rounded-3xl shadow-sm h-fit">
          ...
          <div className="pt-6 space-y-4">
              ...
              <div className="flex flex-wrap gap-1.5 mt-1.5">
                {user.skills && user.skills.length > 0 ? (
                  user.skills.map((skill, idx) => (
                    <span key={idx} className="px-2 py-0.5 bg-slate-100 text-slate-600 text-xs font-medium rounded-lg">
                      {skill}
                    </span>
                  ))
                ) : (
              ...
              <div className="flex flex-wrap gap-1.5 mt-1.5">
                {user.targetCompanies && user.targetCompanies.length > 0 ? (
                  user.targetCompanies.map((company, idx) => (
                    <span key={idx} className="px-2 py-0.5 bg-indigo-50 text-indigo-600 text-xs font-semibold rounded-lg">
                      {company}
                    </span>
                  ))
                ) : (
            ...
            <Link
              to="/profile/edit"
              className="block text-center w-full py-2.5 border border-slate-200 hover:border-indigo-600 hover:text-indigo-600 text-slate-500 text-xs font-bold rounded-xl transition-all"
            >
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="bg-white border border-slate-100 p-6 rounded-3xl shadow-sm">
            <div className="flex items-center justify-between mb-6 pb-4 border-b border-slate-50">
              <h2 className="text-lg font-bold text-slate-800">Recent Session Requests</h2>
              <Link to="/my-requests" className="text-xs font-bold text-indigo-600 hover:underline">
                View all requests
              </Link>
            </div>
            ...
            {loading ? (
              ...
              <div className="text-center py-12">
                ...
                <h3 className="text-sm font-bold text-slate-700">No requests sent yet</h3>
                <p className="text-xs text-slate-400 mt-1 max-w-sm mx-auto">
                  Find a senior/alum working at your target company and submit a placement prep request.
                </p>
                <Link
                  to="/search"
                  className="inline-flex mt-4 px-4 py-2 bg-indigo-600 text-white text-xs font-bold rounded-xl"
                >
                  Start Searching
                </Link>
              </div>
              ...
              <div className="space-y-4.5">
                {requests.slice(0, 5).map((req) => (
                  <div
                    key={req._id}
                    className="p-4 border border-slate-100 rounded-2xl hover:border-slate-200 transition-all flex flex-col sm:flex-row sm:items-center justify-between gap-4"
                  >
                    <div>
                      ...
                      <p className="text-xs text-slate-400 mt-1 bg-slate-50 border border-slate-100 p-2 rounded-lg italic line-clamp-1 font-medium">
                        "{req.requestMessage}"
                      </p>
                    </div>
